...
func listAction(reqenv *base.RequestEnv, msg *tgbotapi.Message, fields wizard.Fields) {
	...
	if fields.FindField(FieldAliasesOrPackages).Data.(string) == Packages {
		items, err = fetchPackages(reqenv.Database, msg.From.ID)
		successTitle = ListStatusSuccessPackages
		noRowsTitle = ListStatusNoRowsPackages
	} else {
		items, err = fetchAliases(reqenv.Database, msg.From.ID)
...
func fetchAliases(db *sql.DB, uid int64) ([]string, error) {
	...
	return fetchCountableList(db, q, uid)
}

func fetchPackages(db *sql.DB, uid int64) ([]string, error) {
	...
	res, err := fetchCountableList(db, q, uid)
	if err == nil {
...
func fetchCountableList(db *sql.DB, query string, uid int64) ([]string, error) {
